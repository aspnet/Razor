        ...
        public override CodeBuilderResult Build()
        {
            ...
            using (writer.BuildNamespace(Context.RootNamespace))
            {
                ...
                using (writer.BuildClassDeclaration("public", Context.ClassName, baseTypes))
                {
                    ...
                    new CSharpHelperVisitor(writer, Host, Context.SourceFile).Accept(Tree.Chunks);
                    new CSharpTypeMemberVisitor(writer, Context.SourceFile).Accept(Tree.Chunks);
                    new CSharpDesignTimeHelpersVisitor(writer, Host, Context.SourceFile).Accept(Tree);
                    ...
                    {
                        new CSharpCodeVisitor(writer, Host, Context.SourceFile).Accept(Tree.Chunks);
                    }
                }
            }
