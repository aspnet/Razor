...
namespace Microsoft.CodeAnalysis.Razor
{
    ...
    internal class BackgroundDocumentGenerator : ProjectSnapshotChangeTrigger
    {
        ...
        private async Task TimerTick()
        {
            try
            {
                ...
                for (var i = 0; i < work.Length; i++)
                {
                    ...
                    try
                    {
                        await ProcessDocument(project, document);
                    }
                    catch (Exception ex)
                    {
                        ReportError(project, document, ex);
                    }
                }
        ...
        private void ReportError(ProjectSnapshot project, DocumentSnapshot document, Exception ex)
        {
