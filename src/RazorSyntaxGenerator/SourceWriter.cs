        ...
        private void WriteGreenFactory(Node nd, bool withSyntaxFactoryContext = false)
        {
            ...
            for (int i = 0, n = nodeFields.Count; i < n; i++)
            {
                ...
                if (field.Type == "SyntaxToken" && field.Kinds != null && field.Kinds.Count > 0)
                {
                    ...
                    if (IsOptional(field))
                    {
                        WriteLine("        case SyntaxKind.Unknown:");
                    }
                    WriteLine("          break;");
                    WriteLine("        default:");
            ...
            if (node is AbstractNode)
            {
                ...
                for (int i = 0, n = nodeFields.Count; i < n; i++)
                {
                    ...
                    if (IsNodeOrNodeList(field.Type))
                    {
                        ...
                        WriteLine($"    public {node.Name} With{field.Name}({fieldType} {UnderscoreCamelCase(field.Name)}) => With{field.Name}Core({UnderscoreCamelCase(field.Name)});");
                        WriteLine($"    internal abstract {node.Name} With{field.Name}Core({fieldType} {UnderscoreCamelCase(field.Name)});");
                ...
                if (baseType != null)
                {
                    ...
                    {
                        WriteLine($"    public new {node.Name} With{baseField.Name}({GetRedFieldType(baseField)} {UnderscoreCamelCase(baseField.Name)}) => ({node.Name})With{baseField.Name}Core({UnderscoreCamelCase(baseField.Name)});");
                    }
        ...
        private void WriteRedWithMethods(Node node)
        {
            for (int f = 0; f < node.Fields.Count; f++)
            {
                ...
                for (int f2 = 0; f2 < node.Fields.Count; f2++)
                {
                    ...
                    {
                        Write("{0}", UnderscoreCamelCase(field2.Name));
                    }
                }
                WriteLine(");");
        ...
        private void WriteRedFactoryWithNoAutoCreatableTokens(Node nd)
        {
            ...
            for (int i = 0, n = nd.Fields.Count; i < n; i++)
            {
                ...
                if (factoryWithNoAutoCreatableTokenFields.Contains(field))
                {
                    ...
                    Write("{0} {1}", GetRedPropertyType(field), UnderscoreCamelCase(field.Name));
